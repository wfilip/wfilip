CREATE OR REPLACE FORCE VIEW V_WYK_WG_INST ("NR_KOM_ZLEC", "NR_INST", "IL_WYK", "ILE_PLAN", "IL_BR", "ZN_WYROBU", "KOLEJN") AS 
  SELECT  L.nr_kom_zlec,L.Nr_inst, 
         count(case when (case when L.wyroznik<>'B' and L.zn_wyrobu<>0 and
                                    (select nvl(max(wartosc),'0') from param_t where kod=128)='0'
                               then E.data_wyk
 ...
 WHERE (L.typ_inst not in ('MON','STR','SPE') or L.nr_warst=1)
   AND (L.typ_inst in ('A C','R C','MON','STR','SPE')
        OR EXISTS
