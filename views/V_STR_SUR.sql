CREATE OR REPLACE FORCE  VIEW "V_STR_SUR1" ("NR_KOM_STR", "NR_SKL", "NR_SKL1", "NR_SKL2", "NR_SKL3", "NR_SKL4", "NR_KAT", "TYP_KAT", "RODZ_SUR", "ZNACZ_PR", "GRUB", "WSP", "NR_WAR", "IL_WAR", "KOD_STR") AS 
select B.nr_kom_str, B.nr_skl, B1.nr_skl nr_skl1, B2.nr_skl nr_skl2, B3.nr_skl nr_skl3, B4.nr_skl nr_skl4,
       ...
       sum(case when B.zn_war='Pol' or K.rodz_sur in ('TAF','LIS') then 1
                when B1.zn_war='Pol' or K1.rodz_sur in ('TAF','LIS') then 1
                when B2.zn_war='Pol' or K2.rodz_sur in ('TAF','LIS') then 1
                when B3.zn_war='Pol' or K3.rodz_sur in ('TAF','LIS') then 1
                when B4.zn_war='Pol' or K4.rodz_sur in ('TAF','LIS') then 1
                else 0 end)
...
--   left join budstr B4 on B3.zn_war in ('Str','Pol') and B4.nr_kom_str=B3.nr_kom_skl
   ...
   where (B.zn_war='Sur' and B1.nr_skl is null or
